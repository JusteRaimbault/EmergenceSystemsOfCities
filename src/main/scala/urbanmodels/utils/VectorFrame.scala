...
object VectorFrame {
  ...
  def drawPolygons(gg: Graphics2D,
                   ...
                   attributeScaleColoring: Seq[Map[String,AnyRef] => Color],
                   frameWidth: Int = 600,
                   frameHeight: Int = 600,
                   margin: Int = 50
                  ): Unit = {
    ...
    polygons.zip(attributeScaleColoring).foreach{case (polys,coloring) =>
      polys.polygons.zip(polys.attributes).foreach{case (poly,attr) =>
        ...
        val (xcoords,ycoords): (Array[Int], Array[Int]) = poly.getCoordinates.map(_.x).zip(poly.getCoordinates.map(_.y)).map(rescaleWindow).map(d => (d._1.toInt,d._2.toInt)).unzip
  ...
  /**
    ...
    * @return
    */
  def attributeGradientScale(attrName: String, gradientColors: (Color,Color) = (Color.LIGHT_GRAY, Color.BLACK))(a: Map[String,AnyRef]): Color = {
