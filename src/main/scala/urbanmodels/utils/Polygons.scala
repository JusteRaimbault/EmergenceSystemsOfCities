...
case class Polygons(
                   ...
                   attributes: Seq[Map[String,AnyRef]] = Seq.empty
                   ) {
  ...
  /**
    ...
    * @return
    */
  def getPolygonByKeyValue(key: String, value: String): Option[(geom.Polygon,Map[String,AnyRef])] = {
  ...
  def foreach(f: (geom.Polygon, Map[String,AnyRef]) => Unit): Unit = polygons.zip(attributes).foreach{case (p,a) => f(p,a)}
...
object Polygons {
  ...
  /**
    ...
    * @return
    */
  def fromGeometries(geoms: Seq[geom.Geometry], attributes: Seq[Map[String,AnyRef]]): Polygons = {
    Polygons(
      geoms.map{g =>
  ...
  def apply(geoms: Seq[(geom.Geometry,Map[String,AnyRef])]): Polygons = fromGeometries(geoms.map(_._1), geoms.map(_._2))
