...
import GraphAlgorithms.ShortestPathMethod
import GraphAlgorithms.{ConnectedComponentsJGraphT, DijkstraJGraphT}
import Network.{IterableDecorator, ShortestPaths, ShortestPath}
...
case class Network(
  ...
  def weakComponentConnect: Network = {
    ...
    def connectClosestComponents(state: (Network,Int)): (Network,Int) = {
      ...
      if(components.size==1) (state._1,1)
        ...
        val (n1min, n2min, _): (Node, Node, Double) = (for {
          c1 <- components
  ...
  def projectionConnect: Network = {
    def addLinkToProjection(state: (Network,Int)): (Network,Int) = {
      ...
      val (projnode,projlink): (Node, Link) = n.projection(state._1.links) //links cannot be empty
      val newLinks: Set[Link] = Set(
...
object Network {
  ...
  /**
    ...
    * @return
    */
  def apply(points: Seq[(Double,Double)]): Network = Network(points.zipWithIndex.map{case (p,i) => Node(i,p)}.toSet, Set.empty[Link])
  ...
  */
  ...
  def networkToGrid(network: Network,footPrintResolution: Double = 1.0,linkwidth: Double = 1.0): Array[Array[Double]] = {
    ...
    def ycor(y: Double): Int = math.max(ymin,math.min(ymax,math.round(y).toDouble)).toInt
